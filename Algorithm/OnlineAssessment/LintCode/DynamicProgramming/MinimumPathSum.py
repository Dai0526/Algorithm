...
整体有序性，依赖于前一个值，所以可用动态规划

初始化 lut[0][0] = grid[0][0]
       lut[0][j] = grid[0][j] + lut[0][j - 1]
       lut[i][0] = grid[i][0] + lut[i-1][0]

动态转移方程：
        lut[i][j] = min(lut[i-1][j], lut[i][j-1]) + grid[i][j]
...
class Solution:
    ...
    def minPathSum(self, grid):
        ...
        for i in range(1, row):
            lut[i][0] = lut[i-1][0] + grid[i][0]
            
        for j in range(1, col):
            lut[0][j] = lut[0][j-1] + grid[0][j]

        for i in range(1, row):
            for j in range(1, col):
                lut[i][j] = min(lut[i-1][j], lut[i][j-1]) + grid[i][j]

        self.print_lut(row, col, lut)

        return lut[row - 1][col - 1]

    def print_lut(self, m, n, lut):
        for i in range(m):
            temp = ""
            for j in range(n):
                temp += str(lut[i][j]) + " "
...
class Solution2:
    ...
    def minPathSum(self, grid):
        ...
        memo = [[sys.maxsize for i in range(c)] for j in range(r)]
        self.traverse(grid, 0, 0, 0, memo)
        
        return memo[r-1][c-1]
    
    def traverse(self, grid, r, c, prev, memo):

        if not self.is_valid(grid, r, c):
            return
        
        if memo[r][c] <= prev + grid[r][c]:
            return
        #print("chech ({},{})".format(r,c))
        memo[r][c] = prev + grid[r][c]

        self.traverse(grid, r + 1, c, memo[r][c], memo)
        self.traverse(grid, r, c + 1, memo[r][c], memo)
