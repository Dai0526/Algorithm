                                        ...
                                        implements BinarySearchTreeADT<T>
{
	...
	public T find(T targetElement) throws ElementNotFoundException 
    {
        if(findNode(targetElement,root)==null)
        	throw new ElementNotFoundException("NoSuchElementException"); 
        BinaryTreeNode<T> temp=new BinaryTreeNode<T>(null);
        return findNode(targetElement,root).getElement();// the return type??
    }
    /**
     * Returns a reference to the specified target element if it is
     * found in this tree.  
    ...
    /**
     * Returns a string representation of this binary tree showing the structure of
	 * the tree using parentheses to show subtrees in an inorder fashion.
     *
    ...
    private String toString(BinaryTreeNode<T> node) {
    	if (node==null)
    		return "";//base case
    	if (node.getLeft()!=null&&node.getRight()==null)//has left child
    		return toString(node.getLeft())+""+ node.getElement()+ ")"+toString(node.getRight());
    	if (node.getRight()!=null&&node.getLeft()==null)//has right child
    		return toString(node.getLeft())+"("+ node.getElement()+ ""+toString(node.getRight());
    	else if(node.getLeft()!=null&&node.getRight()!=null)//has two childred
    		return toString(node.getLeft())+")"+ node.getElement()+ "("+toString(node.getRight());
    	else// toString for leaves
    		return toString(node.getLeft())+"("+ node.getElement()+ ")"+toString(node.getRight());
    }
}
