...
public class BinarySearchTree<T extends Comparable<? super T>> implements Iterable<T> {
	...
	private boolean isBst(BinaryNode<T> node) {
		if (node.left == null && node.right == null)
			return true;
		if (hasLeft(node) && node.left.data.compareTo(node.data) < 0)
			return true;
		if (hasRight(node) && node.right.data.compareTo(node.data) > 0)
			return true;

		return isBst(node.left) && isBst(node.right);
	}

	private boolean hasLeft(BinaryNode<T> node) {
		return node.left != null;
	}

	private boolean hasRight(BinaryNode<T> node) {
	...
	public List<T> getInterval(T min, T max) {
		...
		return getInterval(root, min, max, ls);
	}

	private List<T> getInterval(BinaryNode<T> node, T min, T max, LinkedList<T> ls) {

		if (node.data.compareTo(min) >= 0 && node.data.compareTo(max) <= 0) {
			ls.add(node.data);
			if (hasLeft(node))
				getInterval(node.left, min, max, ls);
			if (hasRight(node))
				getInterval(node.right, min, max, ls);
		}
	...
	private void postOrder(BinaryNode<T> node, LinkedList<T> tempList) {
		if (node != null) {
