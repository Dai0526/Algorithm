...
SELECT name AS WAREHOUSE_NAME, SUM(w.units * vlms.size) AS VOLUME
FROM Warehouse w 
LEFT JOIN
(SELECT product_id AS id, (Width * Length * Height) AS size
FROM Products) AS vlms

ON w.product_id = vlms.id
GROUP BY name;
